(function Ex5() {
	...
	function renderRecords(records) {
		var transforms = {
			"row": {
				"<>": "tr",
				html: "<td>${UserData}</td><td>${Content}</td>",
			},
			"table": {
				"<>": "table",
				border: "1",
				cellPadding: "10",
				html: function table() {
					// json2html for converting json list into row elements
	...
	async function getRecords() {
		...
		// res.ok to check response status is 200 or not
		if (res && res.ok) {
			// res.json to get response body
